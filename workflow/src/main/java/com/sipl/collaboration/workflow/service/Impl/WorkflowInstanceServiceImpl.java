...
import com.sipl.collaboration.core.dao.repository.WorkflowInstanceRepository;
...
import com.sipl.collaboration.core.helper.UserManager;
...
public class WorkflowInstanceServiceImpl implements WorkFlowInstanceService {
    ...
    // throws RuntimeException
    {
        ...
        notificationDto.setMessage("Cancellation Request Created");
    ...
    public ApiResponseDTO<WorkflowInstanceDTO> updateWorkflowInstance(WorkflowInstanceDTO dto) {
        ...
        if (entityToUpdate.getEntityType().equals("TRANSACTION")) {
            Optional<Transactions> requestOrder =
                    transactionRepository.findById(entityToUpdate.getEntityId());
            if (requestOrder.isEmpty()) {
                throw new RuntimeException(
                        "Transaction ID not found with ID: " + entityToUpdate.getEntityId());
            }
            Transactions toBeUpdate = requestOrder.get();
            if(entityToUpdate.getWorkFlowStatus() != WorkFlowStatusEnum.IN_PROGRESS.getCode()) {
                if (entityToUpdate.getWorkFlowStatus()==WorkFlowStatusEnum.APPROVED.getCode()) {
                    toBeUpdate.setStatus(TransactionsStatus.CANCELLED.getCode());
                    toBeUpdate.setCancelledBy(entityToUpdate.getCreatedBy());
                    toBeUpdate.setCancelTime(LocalDateTime.now());
                    toBeUpdate.setIsCancelled(true);
                    toBeUpdate.setCancelReason(entityToUpdate.getReason());
                } else {
                    toBeUpdate.setStatus(TransactionsStatus.IN_PROGRESS.getCode());
                }
            }
            transactionRepository.save(toBeUpdate);
        }
    ...
    public ApiResponseDTO<PagedResponse<WorkflowInstanceDTO>> searchWorkflowInstancesByPagination(
        ...
        try {
            ...
            ApiResponseDTO<WorkflowInstanceDTO> response = validateUser(servletRequest);
            if (response != null) {
            ...
            Long roleId = userRoleMappingEntity.getRole().getId();

            Page<WorkflowInstance> page =
                    workflowInstanceRepository.findByEntityTypeAndStatusAndRole(
            ...
            List<WorkflowInstanceDTO> dtos = page.getContent()
                    .stream()
                    .map(workflowInstanceMapper::toDto)
                    .collect(Collectors.toList());
            populateLepNumbers(dtos);
    ...
    private void populateLepNumbers(List<WorkflowInstanceDTO> dtos) {
        if (dtos == null || dtos.isEmpty()) return;

        List<Long> entityIds = dtos.stream()
                .map(WorkflowInstanceDTO::getEntityId)
                .collect(Collectors.toList());

        List<Transactions> transactions = transactionRepository.findAllById(entityIds);

        Map<Long, Transactions> transactionMap = transactions.stream()
                .collect(Collectors.toMap(Transactions::getId, t -> t));

        for (WorkflowInstanceDTO dto : dtos) {
            Transactions tx = transactionMap.get(dto.getEntityId());
            if (tx == null) {
                log.info("Transaction ID not found with ID: " + dto.getEntityId());
            }
            dto.setLepNumber(tx.getVehicleAllocation().getLepNumber());
        }
    }
